...
class FirstOrderVolterra:
    ...
    def __init__(self, tau_s=0.8, tau_f=0.4, k_1=5.6, v_0=0.02):
        ...
        self.k_1 = torch.tensor([k_1])
        self.v_0 = torch.tensor([v_0])

    def eval(self, var):
        return 1/3. * torch.exp(-0.5 * (var / self.tau_s)) * \
               (torch.sin(torch.sqrt(1. / self.tau_f - 1./(4. * self.tau_s**2)) * var)) / \
               (torch.sqrt(1./self.tau_f - 1./(4. * self.tau_s**2)))
...
class Bold:
    ...
    **Attributes**
        ...
        period    : the monitor's period (For the BOLD monitor, sampling period in milliseconds must be
        an integral multiple of 500. Typical measurment interval (repetition
        time TR) is between 1-3 s.)
    """
    def __init__(self, period=2000, hrf_length=20000, **kwargs):
    ...
    def compute_hrf(self):
        ...
        self._stock_time = torch.arange(0.0, stock_time_max, stock_time_step)  # [s]
    ...
    def _config_time(self, n_nodes, dt=0.1, bold_inner=None, bold_stock=None):
        self.dt = dt
        self.istep = round(self.period / self.dt)
        self.compute_hrf()
        sample_shape = self.voi_idx.shape[0], n_nodes
        if bold_inner != None:
            self._interim_stock = bold_inner
        else:
            self._interim_stock = torch.zeros((self._interim_istep,) + sample_shape)
        if bold_stock != None:
            self._stock = bold_stock
        else:
            self._stock = torch.zeros((self._stock_steps,) + sample_shape)

    def config_for_sim(self, n_voi, n_nodes, dt=0.1, bold_inner=None, bold_stock=None):
        ...
        self._config_time(n_nodes, dt, bold_inner, bold_stock)

    def sample(self, step, state):  # state at single time spot
        # Update the interim-stock at every step (state.shape = (n_vars, n_regions))
        ...
        if step % self._interim_istep == 0:
            avg_interim_stock = torch.mean(self._interim_stock, dim=0)
        ...
        if step % self.istep == 0:
            time = step * self.dt
            # hrf = torch.as_tensor(np.roll(self.hrf,
            #                               ((step // self._interim_istep % self._stock_steps) - 1),
            #                               axis=1))
            hrf = torch.roll(self.hrf, ((step // self._interim_istep % self._stock_steps) - 1), dims=1)
            # if isinstance(self.hrf_kernel, FirstOrderVolterra):
            k1_V0 = self.hrf_kernel.k_1 * self.hrf_kernel.v_0
            bold = (torch.stack([torch.mm(hrf, self._stock[:, 0, :]),
                                 torch.mm(hrf, self._stock[:, 1, :])], dim=1) - 1.0) * k1_V0
...
#     # t_seq = torch.arange(0.0, 20, 20/5000)
